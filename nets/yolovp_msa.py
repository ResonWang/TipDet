...
class YOLOXHead_PriorBox_RegionalMSA1_OF_KeyFrame_Gray_or_OF_fineFeat(nn.Module):
    ...
    def forward(self, xin_batch):
        batch_size = xin_batch[0].shape[0]
        gframe = self.group_num
        MSA_groups = batch_size // gframe
        output_yolox_batch = []
        fc_outputs_batch = []
        pred_results_batch = []
        pred_idx_batch = []
        feat_num = 2 # len(xin_batch) - 1

        for i in range(MSA_groups):
            output_yolox = []
            outputs_decode = []
            before_nms_features = []
            xin = [0] * feat_num
            ...
            for j in range(feat_num):                                                                       # 遍历每个特征层
                xin[j] = xin_batch[j][i * gframe:(i + 1) * gframe]
                xin_gray[j] = xin_batch[j][i * gframe:(i + 1) * gframe, 0:xin_batch[j].shape[1] // 2, ...]  # 前64个通道 [(16,64,80,80), (16,128,40,40)]
                xin_OF[j] = xin_batch[j][i * gframe:(i + 1) * gframe, xin_batch[j].shape[1] // 2:, ...]

            # dark1: 4x16x320x320, dark2: 4x32x160x160
            dark1, dark2, stem_of, dark2_of = xin_batch[3][i * gframe:(i + 1) * gframe], xin_batch[4][i * gframe:(i + 1) * gframe], xin_batch[5][i * gframe:(i + 1) * gframe], xin_batch[6][i * gframe:(i + 1) * gframe]
            ...
            for k, (cls_conv, cls_conv2, reg_conv, stride_this_level, x) in enumerate(
                ...
                x = self.stems[k](x)  # (16,128,80,80)-->(16,64,80,80)

                reg_feat = reg_conv(x)  # (4,64,80,80)-->(4,64,80,80)
                cls_feat = cls_conv(x)  # (4,64,80,80)-->(4,64,80,80)  VID cls

                # this part should be the same as the original model
                obj_output = self.obj_preds[k](reg_feat)  # (4,64,80,80) --> (4,1,80,80)   obj和reg不共享分支了
                reg_output = self.reg_preds[k](reg_feat)  # (4,64,80,80) --> (4,4,80,80)
                cls_output = self.cls_preds[k](cls_feat)

                output = torch.cat([reg_output, obj_output, cls_output], 1)  # (4,7,80,80)
                output_yolox.append(output.clone().detach())  # 深拷贝
                # ---------------------------------以上为原始的yolox head部分------------------------------------------------#

                output_decode = torch.cat([reg_output, obj_output.sigmoid(), cls_output.sigmoid()], 1)
                before_nms_features.append(reg_feat)                                                               # list : [(4,64,80,80),(4,64,40,40)]
                outputs_decode.append(output_decode)                                                               # [(4,7,80,80),(4,7,40,40),(4,7,20,20)]

            # --------------------------------以下为yolov 的分类头部分------------------------------  ------------------#
            self.hw = [x.shape[-2:] for x in outputs_decode]                                                       # [(80,80),(40,40)]
            outputs_decode = torch.cat([x.flatten(start_dim=2) for x in outputs_decode], dim=2).permute(0, 2, 1)   # (4,8000,7)  8400=80x80+40x40+20x20
            decode_res = self.decode_outputs(outputs_decode, dtype=xin[0].type())                                  # 解码为坐标值(640尺寸下)和概率值
            pred_result, pred_idx = self.postpro_woclass(decode_res, nms_thre=self.nms_thresh, topK=self.Afternum)
            """
            提取大尺寸特征图的细粒度特征
            """
            dark2_feat = self.find_index_for_dark2_ROIPooling(dark2, pred_idx, pred_result)  # (10,32x7x7=1568)
            dark2_feat = self.linear1568to64(dark2_feat)                                     # (10,1568)-->(10,xx)

            """
            提取先验框的OF_PATH特征，用于CA
            """
            before_nms_features_gray = [xin_OF[0], self.gray_128to64(xin_OF[1])]                                                   # [(4,64,80,80),(4,64,40,40)]
            reg_feat_flatten_gray = torch.cat([x.flatten(start_dim=2) for x in before_nms_features_gray], dim=2).permute(0, 2, 1)  # [b,features,channels]  (4,8000,128) cls_feat2的预测集合
            features_reg_gray = self.find_feature_score(reg_feat_flatten_gray, pred_idx)                                           # 每帧只保留10个框
            features_reg_last_frame_gray = features_reg_gray[(self.group_num - 1) * self.simN:]                                    # (p,64)
            """
            时空cube 
            """
            spatiotemporal_feat = self.find_index_for_spatialTemporal_MSA_and_fetch(before_nms_features_gray[0],
                                                                                    before_nms_features_gray[1],
                                                                                    pred_idx)
            spatiotemporal_feat_last_frame = spatiotemporal_feat[-1]
            """
            提取先验框的混合流特征
            """
            reg_feat_flatten = torch.cat([x.flatten(start_dim=2) for x in before_nms_features], dim=2).permute(0, 2, 1)  # [b,features,channels]  (4,8000,128) cls_feat2的预测集合
            features_reg = self.find_feature_score(reg_feat_flatten, pred_idx)                                           # 每帧只保留10个框
            features_reg_last_frame = features_reg[(self.group_num - 1) * self.simN:]                                    # (10,128)

            MSA_feat = []
            for ii in range(self.simN):                                                                    # 遍历group最后一帧每一个框,每个框和自己的邻域attn
                feat_one_box = features_reg_last_frame_gray[ii].unsqueeze(0)                               # (1,64)
                mix_feat = torch.cat([feat_one_box, spatiotemporal_feat_last_frame[ii]], 0).unsqueeze(0)   # (1, 4x9X9+1,64) = (1,325,64)
                MSA_feat.append(mix_feat)
            MSA_feat = torch.cat(MSA_feat,0)                                                  # (p=10,325,64)
            MSA_feat = self.mutualMSA_parallel(MSA_feat).squeeze(0).squeeze(1)                # (p,101,64) --> (p,64)
            MSA_feat = torch.cat([features_reg_last_frame, dark2_feat, MSA_feat], 1)          # (p,64) + (p,64) + (p,64) + (p,64) --> (p,256) 原始双流特征+灰度时空MSA特征

            # -------------------------------------------------------------------------------
            fc_output = self.linear_pred(MSA_feat)                           # (10,128) --> (10,1)  obj
            fc_output = fc_output.unsqueeze(0).repeat(self.group_num, 1, 1)  # (10,1)-->(1,10,1)-->(4,10,1)

            output_yolox_batch.append(output_yolox)                          # list 3: [(4,6,80,80),(4,6,40,40)]
            fc_outputs_batch.append(fc_output)
            pred_results_batch.append(pred_result)
            pred_idx_batch.append(pred_idx)
        ...
        if feat_num == 3:
            ...
            f2 = []
            f3 = []
            pred_result_temp = []        # [bsx(10,6)]
            pred_idx_temp = []           # [bsx(10,)]
            for i in range(MSA_groups):  # batch 里面的组数
                f1.append(output_yolox_batch[i][0])
                f2.append(output_yolox_batch[i][1])
                f3.append(output_yolox_batch[i][2])
                pred_result_temp = pred_result_temp + pred_results_batch[i]
                pred_idx_temp = pred_idx_temp + pred_idx_batch[i]
            output_yolox = [torch.cat(f1, 0), torch.cat(f2, 0),
                            torch.cat(f3, 0)]  # 三个特征层的预测头输出 [(bs,35,80,80),(bs,35,40,40),(bs,35,20,20)]
        ...
        if feat_num == 2:
            ...
            for i in range(MSA_groups):        # batch 里面的组数
                ...
                pred_result_temp = pred_result_temp + pred_results_batch[i]
                pred_idx_temp = pred_idx_temp + pred_idx_batch[i]
            output_yolox = [torch.cat(f1, 0), torch.cat(f2, 0)]  # 三个特征层的预测头输出 [(bs,6,80,80),(bs,6,40,40),(bs,6,20,20)]

        pred_result = pred_result_temp                           # [bsx(10,37)]
        fc_output = torch.cat(fc_outputs_batch, 0)               # (bs,10,1)
        pred_idx = pred_idx_temp                                 # [bsx(10,)]

        return output_yolox, fc_output, pred_result, pred_idx
